...
import { CertificatePayload, CertificateProof, ReplayOperation } from '@/lib/certificate';
import { ValidationStatus } from '@/lib/telemetry';
import { signPayload, stableStringify, verifyPayloadSignature } from '@/lib/server-signing';
...
const buildUnsignedProof = (
  ...
  input: Pick<CreateCertificateInput, 'validationStatus' | 'riskScore' | 'confidence'>
) => {
  ...
  return {
    ...
    riskScore: Number.isFinite(input.riskScore) ? Math.round(input.riskScore as number) : null,
    confidence: Number.isFinite(input.confidence) ? Number((input.confidence as number).toFixed(3)) : null,
  };
};

const buildLogEntryHash = (certificateId: string, prevHash: string | null, signature: string) =>
  sha256Hex(
    stableStringify({
      certificateId,
      prevHash,
...
const isValidationStatus = (value: unknown): value is ValidationStatus =>
  value === 'VERIFIED_HUMAN' || value === 'SUSPICIOUS' || value === 'LOW_EFFORT' || value === 'INSUFFICIENT_DATA';
...
const parseProof = (raw: unknown): CertificateProof | null => {
  ...
  return {
    ...
    telemetryDigestSha256,
    issuedAt,
    validationStatus: isValidationStatus(validationStatus) ? validationStatus : null,
    riskScore: typeof candidate.riskScore === 'number' ? candidate.riskScore : null,
    confidence: typeof candidate.confidence === 'number' ? candidate.confidence : null,
    signature,
    logEntryHash: typeof candidate.logEntryHash === 'string' ? candidate.logEntryHash : null,
    prevLogEntryHash: typeof candidate.prevLogEntryHash === 'string' ? candidate.prevLogEntryHash : null,
  };
};

const hydratePayload = (result: typeof certificates.$inferSelect): CertificatePayload => ({
  id: result.id,
  title: result.title,
  ...
  replay: normalizeReplay(Array.isArray(result.replay) ? (result.replay as ReplayOperation[]) : []),
...
export const verifyCertificatePayload = async (payload: CertificatePayload): Promise<CertificateVerificationResult> => {
  if (!payload.proof) {
    return { isValid: false, reason: 'Missing proof bundle.' };
  }

  const unsignedProof = buildUnsignedProof(payload, {
    validationStatus: isValidationStatus(payload.proof.validationStatus) ? payload.proof.validationStatus : undefined,
  ...
  const signatureValid = verifyPayloadSignature(unsignedProof, payload.proof.signature, 'MINDPRINT_CERTIFICATE_SECRET');
  if (!signatureValid) {
  ...
  const expectedLogHash = buildLogEntryHash(payload.id, payload.proof.prevLogEntryHash, payload.proof.signature);
  if (logRow.entryHash !== expectedLogHash || payload.proof.logEntryHash !== expectedLogHash) {
    return { isValid: false, reason: 'Transparency log hash mismatch.' };
  }

  if ((logRow.prevHash ?? null) !== (payload.proof.prevLogEntryHash ?? null)) {
    return { isValid: false, reason: 'Transparency chain predecessor mismatch.' };
  }
