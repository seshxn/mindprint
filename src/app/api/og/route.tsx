...
import { getCyberpunkPalette } from "@/lib/certificate";
import {
...
const safeGetCertificateRecord = async (id: string) => {
  try {
    ...
    console.error(
      "[api/og] Failed to load certificate record from database:",
      error,
    );
    return null;
  }
...
export const GET = async (request: NextRequest) => {
  try {
    ...
    if (!verification.isValid) {
      throw new Error(
        `Certificate verification failed: ${verification.reason || "invalid proof."}`,
      );
    }
    ...
    const origin =
      url.searchParams.get("origin") || url.origin || "https://mindprint.app";
    ...
    const excerpt =
      payload.text.length > 260
        ? `${payload.text.slice(0, 257)}...`
        : payload.text;
    ...
    const message =
      error instanceof Error ? error.message : "Unknown OG rendering error";
    return new ImageResponse(
      <div
        style={{
          width: "100%",
