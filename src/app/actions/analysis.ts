...
export const analyzeSession = async (sessionId: string) => {
  ...
  if (eventsRecords.length === 0) {
    return {
      error: "NO_TELEMETRY",
      message:
        "No telemetry data found for this session. Try writing something first!",
    };
  }
  ...
  if (!apiKey) {
    return {
      ...
      message:
        "API key not configured. Please add GOOGLE_API_KEY to your .env file.",
    };
  }
